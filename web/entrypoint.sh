...
APP_PORT_NUMBER=${APP_PORT_NUMBER:-${django_port}}
...
test -w /etc/nginx/conf.d/app.conf && rm /etc/nginx/conf.d/app.conf
# Linking Nginx configuration file
ln -s -f /etc/nginx/sites-available/app$ssl /etc/nginx/conf.d/app.conf

# Setup app host and port on configuration file
sed -i "s/{%APP_HOST%}/${APP_HOST}/g" /etc/nginx/conf.d/app.conf
sed -i "s/{%APP_PORT%}/${APP_PORT_NUMBER}/g" /etc/nginx/conf.d/app.conf
